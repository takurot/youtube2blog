...
現在、`youtube2blog.py` で生成されたブログ記事の主要なポイントに基づいて、元の YouTube 動画から短いクリップ動画を生成する機能を追加開発中です。

### 目的

ブログ記事のハイライト部分に対応する動画クリップを自動で作成し、コンテンツの再利用性を高めます。

### 現在の実装状況 (`create_clips_from_blog.py`)

- **ブログ記事の解析**: 生成されたブログ記事 (`<日付>_blog_article_<動画ID>.txt`) を読み込み、主要なポイント（段落ごと）を抽出します。
- **YouTube トランスクリプトの取得**: 元の YouTube 動画の完全なトランスクリプト（タイムスタンプ付き）を取得します。
- **基本的なファイル処理**: 動画IDの抽出など、関連するファイル情報を扱います。

### 今後の開発予定

- **ポイントとトランスクリプトのアライメント**: ブログ記事の各ポイントを、トランスクリプト内の対応する時間範囲に正確にマッピングするロジックを実装します（テキスト類似性分析などを使用予定）。
- **動画のダウンロードと編集**: `yt-dlp` や `ffmpeg` を使用して、特定された時間範囲に基づいて元の動画からセグメントを切り出します。
- **クリップへの情報付加（オプション）**: 切り出したクリップに、関連するブログのテキストをオーバーレイ表示する機能などを検討します。

この機能はまだ開発の初期段階にあり、`create_clips_from_blog.py` スクリプトとして部分的に実装が進められています。
