...
def generate_blog_article_zh(
    ...
    """文字起こしデータを基にOpenAI APIを使って中国語のブログ記事を生成"""
    ...
    intro_to_transcript_processing = "请基于以下 YouTube 视频的逐字稿，撰写一篇中文解读型博客文章。"
...
"""
    ...
    messages = [
        {
            "role": "system",
            "content": "你是一名专业中文博客作者，能够严格按照指示生成高质量内容，并以指定的 JSON 结构输出。",
        },
        {"role": "user", "content": prompt_content},
    ]

    try:
        print("LLMに中国語ブログ記事の生成をリクエストします...")
...
def _generate_output_filenames(youtube_url: str, is_shorts: bool) -> dict[str, str]:
    ...
    base_name = f"{today}_blog_local_zh_{video_id}"
...
def main():
    parser = argparse.ArgumentParser(
        description="YouTubeからローカルで文字起こしを生成し、中国語のブログ記事を作成するスクリプト"
    )
    ...
    print(f"\n=== YouTube動画からローカルで文字起こしを生成（中国語ブログ） ===")
    ...
    print("\nブログ記事（中国語）を生成中...")
    blog_article_text, error_message = generate_blog_article_zh(
        transcript_data,
        args.youtube_url,
        no_timestamps=True,
        language="zh",
